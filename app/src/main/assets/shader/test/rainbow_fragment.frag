...
vec4 getRainbow(vec2 uv, float wave, float startY, float width) {
    vec4 color;
...
void mainImage( out vec4 O, in vec2 U ) {
    ...
    vec2 pos = vec2(uLandmarkX[98], (uLandmarkY[98] + uLandmarkY[102])*0.5);
    vec2 vecToPoint = pos - uv;

    float amp = 0.25;
    float wave = amp * sin(U.y- iGlobalTime*10.) -U.x + (pos.x * 2. -1.)* 5.;

    O = getRainbow(uv, wave, pos.y, width);
}
